 ...
 * 
 */
public class LoginGesturePatternUtils {
	...
	public LoginGesturePatternUtils(Context context) {
	 ...
	 * @return The pattern.
	 */
	public static List<LoginGesturePatternView.Cell> stringToPattern(String string) {
		List<LoginGesturePatternView.Cell> result = new ArrayList<LoginGesturePatternView.Cell>();

		final byte[] bytes = string.getBytes();
		for (int i = 0; i < bytes.length; i++) {
			byte b = bytes[i];
			result.add(LoginGesturePatternView.Cell.of(b / 3, b % 3));
		}
		return result;
	}
	 ...
	 */
	public static String patternToString(List<LoginGesturePatternView.Cell> pattern) {
		if (pattern == null) {
			return "";
		}
		...
		for (int i = 0; i < patternSize; i++) {
			LoginGesturePatternView.Cell cell = pattern.get(i);
	 ...
	 */
	public void saveLockPattern(List<LoginGesturePatternView.Cell> pattern) {
		// Compute the hash
		final byte[] hash = LoginGesturePatternUtils.patternToHash(pattern);
		try {
	 ...
	 */
	private static byte[] patternToHash(List<LoginGesturePatternView.Cell> pattern) {
		if (pattern == null) {
			return null;
		}
	 ...
	 */
	public boolean checkPattern(List<LoginGesturePatternView.Cell> pattern) {
		try {
			...
			return Arrays.equals(stored,
					LoginGesturePatternUtils.patternToHash(pattern));
		} catch (FileNotFoundException fnfe) {
			return true;
		} catch (IOException ioe) {
			return true;
