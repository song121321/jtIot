...
import song.honey.ui.LoginGesturePatternView.DisplayMode;
...
public class LoginGestureCreatePasswordActivity extends Activity implements
	...
	protected List<LoginGesturePatternView.Cell> mChosenPattern = null;
	 ...
	 */
	private final List<LoginGesturePatternView.Cell> mAnimatePattern = new ArrayList<LoginGesturePatternView.Cell>();

	/**
	 * The states of the left footer button.
	 */
	enum LeftButtonMode {
		Cancel(R.string.lockpattern_pass, true), CancelDisabled(
				R.string.lockpattern_pass, false), Retry(
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.logingesture_password_create);
		MyApplication.getInstance().addActivity(this);
		// 初始化演示动画
		mAnimatePattern.add(LoginGesturePatternView.Cell.of(0, 0));
		mAnimatePattern.add(LoginGesturePatternView.Cell.of(0, 1));
		mAnimatePattern.add(LoginGesturePatternView.Cell.of(1, 1));
		mAnimatePattern.add(LoginGesturePatternView.Cell.of(2, 1));
		mAnimatePattern.add(LoginGesturePatternView.Cell.of(2, 2));
		...
		if (savedInstanceState == null) {
			...
			if (patternString != null) {
				mChosenPattern = LoginGesturePatternUtils
	...
	private void updatePreviewViews() {
		...
		for (LoginGesturePatternView.Cell cell : mChosenPattern) {
			Log.i("way", "cell.getRow() = " + cell.getRow()
					+ ", cell.getColumn() = " + cell.getColumn());
			mPreviewViews[cell.getRow()][cell.getColumn()]
					.setBackgroundResource(R.drawable.logingesture_create_grid_selected);
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		if (mChosenPattern != null) {
			outState.putString(KEY_PATTERN_CHOICE,
					LoginGesturePatternUtils.patternToString(mChosenPattern));
		}
	}
	...
	protected LoginGesturePatternView.OnPatternListener mChooseNewLockPatternListener = new LoginGesturePatternView.OnPatternListener() {
		...
		public void onPatternDetected(List<LoginGesturePatternView.Cell> pattern) {
			...
			if (mUiStage == Stage.NeedToConfirm
				...
				if (pattern.size() < LoginGesturePatternUtils.MIN_LOCK_PATTERN_SIZE) {
					updateStage(Stage.ChoiceTooShort);
				} else {
					mChosenPattern = new ArrayList<LoginGesturePatternView.Cell>(
							pattern);
	...
	private void updateStage(Stage stage) {
		...
		if (stage == Stage.ChoiceTooShort) {
			mHeaderText.setText(getResources().getString(stage.headerMessage,
					LoginGesturePatternUtils.MIN_LOCK_PATTERN_SIZE));
		} else {
	...
	private void saveChosenPatternAndFinish() {
		LoginGesturePatternUtils lgp = new LoginGesturePatternUtils(this);
		lgp.saveLockPattern(mChosenPattern);
		showToast(getString(R.string.gesture_password_create_passsucess));
		startActivity(new Intent(this, LoginGestureUnlockPasswordActivity.class));
		finish();
	}
}
